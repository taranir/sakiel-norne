 ...
 * @author Taranir
 */
public class BlacksmithState extends BasicGameState {
    ...
    TiledMap currentMap, blacksmithMap;
...
//Written by Tian
    ...
    public void checkEncounter(GameContainer gc, StateBasedGame g) {
        for (int j = npcs.size() - 1; j >= 0; j--) {
            if (collision(j) && gc.getInput().isKeyPressed(Input.KEY_ENTER)) {
                ...
                GameData.fromstate = Game.BLACKSMITH;
    ...
    public void enter(GameContainer gc, StateBasedGame g) throws SlickException {
        ...
        Game.currentMusic = Game.blacksmith;
    ...
    public void init(GameContainer gc, StateBasedGame g) throws SlickException {
        blacksmithMap = new TiledMap("graphics/maps/maps/blacksmithroom.tmx");
        currentMap = blacksmithMap;
...
//draws a polygon that will be used for the npcs
        ...
        doorPolygon.setX(272);
        doorPolygon.setY(250);
...
//lets user hold down arrow keys to move
    ...
    /*
     * If the PC is near an NPC and presses enter, they begin dialogue. Also movement via arrow keys and
     * press S to go into the statistics viewing screen.
     * If PC is near door and presses enter, exits room.
     * Written by Tian
     */
    public void update(GameContainer gc, StateBasedGame g, int delta) throws SlickException {
        ...
        if (gc.getInput().isKeyPressed(Input.KEY_LEFT) && pc_x > 272) {
            pc_x -= 16;
        }
        if (gc.getInput().isKeyPressed(Input.KEY_RIGHT) && pc_x < 368) {
            pc_x += 16;
        }
        if (gc.getInput().isKeyPressed(Input.KEY_UP) && pc_y > 160) {
            pc_y -= 16;
        }
        if (gc.getInput().isKeyPressed(Input.KEY_DOWN) && pc_y < 224) {
            pc_y += 16;
        }
